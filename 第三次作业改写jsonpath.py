...
for page in range(0, 3):
    ...
    # print(res.text)
    result = res.json()  # 第一步
    # 通过jsonpath取值（不需要考虑位置，直接定位到对应的key对应value值)
    title = jsonpath.jsonpath(result, '$..title')
    salary = jsonpath.jsonpath(result, '$..salary')
    compName = jsonpath.jsonpath(result, '$..compName')
    # print(title, salary, compName)
    for index,value in enumerate(title):
        print(value, salary[index], compName[index])
    break