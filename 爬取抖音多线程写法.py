...
for page in range(1,3):
   # arg1='device_platform=webapp&aid=6383&channel=channel_pc_web&sec_user_id=MS4wLjABAAAA0GXxOnrNe6nYYhDL5Ny_OFW85Xu-D8mqar8wJ_SDwB4&max_cursor=1705674491000&locate_item_id=7320696181170965798&locate_query=false&show_live_replay_strategy=1&need_time_list=0&time_list_query=0&whale_cut_token=&cut_version=1&count=18&publish_video_strategy_type=2&pc_client_type=1&version_code=170400&version_name=17.4.0&cookie_enabled=true&screen_width=1280&screen_height=720&browser_language=zh-CN&browser_platform=Win32&browser_name=Edge&browser_version=120.0.0.0&browser_online=true&engine_name=Blink&engine_version=120.0.0.0&os_name=Windows&os_version=10&cpu_core_num=8&device_memory=8&platform=PC&downlink=10&effective_type=4g&round_trip_time=50&webid=7307940414392387084'
    arg1 = f'device_platform=webapp&aid=6383&channel=channel_pc_web&sec_user_id=MS4wLjABAAAATOBMmLZvb0HxbtqKDfyCIZTuglI68TaDEzgoBhswrCU&max_cursor={max_cursor}&locate_query=false&show_live_replay_strategy=1&need_time_list=1&time_list_query=0&whale_cut_token=&cut_version=1&count=18&publish_video_strategy_type=2&pc_client_type=1&version_code=170400&version_name=17.4.0&cookie_enabled=true&screen_width=1366&screen_height=768&browser_language=zh-CN&browser_platform=Win32&browser_name=Chrome&browser_version=119.0.0.0&browser_online=true&engine_name=Blink&engine_version=119.0.0.0&os_name=Windows&os_version=10&cpu_core_num=8&device_memory=8&platform=PC&downlink=10&effective_type=4g&round_trip_time=50&webid=7282735985581229583&msToken=nDdmYSnP_7MaZ9Qm_LlPgVxxxnAUCtH_K6VjP3sO9mCAizGJtPnW8yhgVeIbOjVZCH4_zHyUPLOI1uVwjN0P463mi_4-xvzjl0GLAiBkcJOqjzoUWeOop4JjE1mNdUOo_TdC'
    #=DFSzswVOJhsANcd9tE-pemD4OF1G
    with open('douyin.js','r',encoding='UTF-8') as f:
        jsdata = f.read()
    ctx = execjs.compile(jsdata)
    sign=ctx.call('window.yuan', arg1)
    # print(post_param1)
    result='&X-Bogus='+sign
    data_url=url+arg1+result
    res=requests.get(data_url,headers=my_headers,cookies=cookie_dict)
    # print(res.text)
...
def get_one_video(url,c):
    res=requests.get(url,cookies=cookie_dict,headers=my_headers)
    #文件io
    with open(f"./抖音视频/{c}{name_list[c-1]}.mp4",'wb') as f:
        f.write(res.content)
    print(f"{c}{name_list[c-1]}.mp4下载成功！")
c = 0
t_list = []
for url in url_list:
...
# 使用列表推导式
# url_list= [
#     (i["preview_title"].split('#')[0], requests.get(i["video"]["play_addr"]["url_list"][0]).content)
#     for i in aweme_list
# ]
# print(url_list)
# 处理视频数据列表
# for title, video_content in video_data_list:
#     with open(f'抖音视频/{title}{num}.mp4', 'wb') as f:
#         f.write(video_content)
#         print(f'{title}{num}.mp4 下载完成')
