...
class WBSpider(object):
    ...
    def main(self, user_url, pass_wd):
        ...
        prox = ''
        response = session.get(url_1, headers=headers_1, verify=False).content.decode()
        # print(response)
        ...
        """
                ...
                """
        prox = ''
        response = session.get(start_url, headers=headers_2, verify=False).json()
        ...
        print(self.parse_response_data(response,i))
        i+=1
        """参数传递，方法回调"""
    ...
    def parse_page_func(self, data, weibo_id, headers_2, i):
        ...
        prox = ''
        response = session.get(start_url,  headers=headers_2, params=data, verify=False).json()
        """提取翻页的max_id"""
        max_id = response['data']['max_id']
        """提取翻页的max_id_type"""
        max_id_type = response['data']['max_id_type']
